...
Notes
-----
This module provided as material for Phase 2 of the exam project for DM562, DM857, DS830 (2022). 
"""
...
class Visualiser:
  ...
  def __init__(self:Visualiser, 
               patches:List[BasePatch], rows:int, cols:int,
               grid_lines = False, grid_markings = False,
    ...
    grid_lines, grid_markings : bool, default = False
      Control whether grid lines and coordinate labels are displayed.
    ...
    # add data raster
    self._im = ax.imshow(self.__data(), cmap='binary',vmin=0,vmax=15, animated=True)
    ...
    for patch in patches:
      if isinstance(patch,CellPatch):
        ...
        v = '%'
      lbls.append(ax.text(patch.col(),patch.row(), v, 
    ...
    # blitter
    self._bm = _BlitManager(fig.canvas, [self._im] + hlns + vlns + lbls)  
